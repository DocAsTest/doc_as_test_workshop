...
Tant que le texte généré est le même, le test est réussi.
Sinon, quelque chose a changé et doit être vérifié.

Dans cet atelier, nous travaillerons sur un projet que nous testerons et documenterons.
Vous pouvez utiliser n'importe lequel de vos projets pour cet atelier.
Le plus important est d'être à l'aise avec votre environnement de développement.
Les contraintes sont que vous devez être en mesure d'exécuter des tests sur ce projet et vous devez être en mesure d'écrire un fichier sur votre disque à partir d'eux.
Il pourrait également être intéressant (mais pas nécessairement) de pouvoir le pousser dans un référentiel github public pour publier la documentation finale et la partager avec d'autres participants.

Si vous n'avez pas de projet, vous pouvez cloner ce dépôt : link:https://github.com/sfauvel/Parrot-Refactoring-Kata[]. 
Il s'agit d'un clone de celui du référentiel https://github.com/emilybache[Emily Bache].

Ce kata est disponible dans de nombreuses langues, vous trouverez donc probablement votre préféré. 
Nous avons configuré le projet java avec un outillage avancé. 
Les projets python et javascript sont également configurés pour démarrer rapidement.
...
Préférez une installation sur votre IDE pour visulaiser le résultat lors du développement.
...
Pour cela, on va créer un fichier `workshop.adoc`. 
L'extension `adoc` est utilisée pour les fichiers au format `{asciidoc}`.

On va y décire l'atelier dans lequel nous sommes en y mettant les informations suivantes:
...
Vous trouverez des exemples basiques sur la link:asciidoc.{page_extension}[page d'exemple] ou , de manière plus complète, sur la page link:https://docs.asciidoctor.org/asciidoc/latest/syntax-quick-reference/[syntax-quick-reference] du site `{asciidoc}`.
...
====
L'objectif est ici est de valider que la documentation est cohérente avec le comportement de l'application.
Pour cela, nous allons nous appuyer sur notre framework de test habituel.
Chaque test génerera le contenu d'un chapitre et vérifiera qu'il produit le même contenu que la version validée de celui çi.
C'est une combinaison des techniques de `Living documentation` de `Golden Master`.
====
...
Choisissez-en une rapide à mettre en place en fonction de votre contexte.
...
* *<<_script_bash_et_git>>*: Vous n'avez qu'à générer, depuis vos tests, votre document dans votre repo Git et un script bash fourni fera la reste. C'est *très simple mais la validation est faite par le script et non par vos tests*.wlwl. 


==== Approvals

Vous pouvez utiliser la librairie https://approvaltests.com/[Approvals] qui est faite pour ça et qui est disponible pour plusieurs languages.

[NOTE]
====
Dans le kata `Parrot`, nous avons configuré cette bibliothèque pour les projets Java, Javascript, Typescript et Python.
====

Si vous devez le configurer pour votre projet ou pour d'autres langages, vous devez spécifier que vous souhaitez générer des fichiers de sortie avec une extension `.adoc` et non `.txt`.
...
Tant que votre contenu est différent de ce fichier, un fichier `received` est généré pour pouvoir comparer le résultat avec celui attendu. 
...
vous pouvez rapidement écrire une méthode qui fait l'équivalent et qui est suffisante pour nos besoins.

Vous trouverez ci-dessous l'algorithme d'une méthode de validation à appeler à la fin de vos tests:
...
Vous pouvez vous contenter de laisser Git gérer les modifications des documents.
Nous fournissons un script `checkDocInFolder.sh` dans le dossier `scripts`
qui appelle des commandes Git pour vérifier si les fichiers du répertoire passé en paramètre ont changé. 
Si c'est las cas, chaque fichier modifié est considéré comme un test en échec.
...
====
Nous allons vous guider à travers un cheminement basé sur le https://github.com/sfauvel/Parrot-Refactoring-Kata[kata de refactoring Parrot].
Vous pouvez tout aussi bien utiliser un de vos projets et adaptez les étapes à votre contexte.
L'important est d'expérimenter l'approche et de se faire une idée de l'utilisation.
====
...
Elle doit-être lisibile, compréhensible et contenir toutes les informations nécessaires pour valider que cela fonctionne ?
. Voyez chaque test comme l'écriture d'un chapitre et pas seulement comme la validation d'un cas isolé.
Vous pouvez regroupez dans un seul test des cas que vous auriez habituellement séparés.
. N'oubliez pas que vous n'êtes pas en train de faire du TDD.
Ne vous laissez pas restreindre par lappplication de cette démarche.
. Vous n'êtes pas obligé d'approuver le document systématiquement avant de lancer le test.
Vous pouvez attendre d'obtenir le résultat qui vous convient.
...
. Afficher la liste des perroquets existant, si possible, en parcourant la liste par programmation.
. Ajouter une image pour chaque perroquet (images disponibles dans le dossier `image`).
. Faire un tableau avec la vitesse de chaque perroquet pour un jeu de valeurs d'entrée fixé et que vous indiquerez dans le document.
...
Nous allons modéliser une `zone` dans laquelle les perroquets pourront voyager. 
Dans cette `zone`, il y aura à différents endroits des `sources d'énérgie` qui auront une pissance comprise entre 0 et 2.
Le perroquet aura également un emplacement dans cette `zone`.
Le perroquet pourra se déplacer dans les différentes direction sans pouvoir sortir de la `zone`.
A chaque mouvement, son voltage diminue de 0,1. 
Lorsqu'il passe sur une source d'énergie, il augmente son voltage de la totalité de la source d'énergie et celle çi disparait. 

. Affichez une table qui va représenter la `zone`.
. Montrez les sources d'énergie et leur puissance.
. Affichez l'emplacement du perroquet
. Documentez la fonction de déplacement et l'évolution du voltage du perroquet.
...
. Créez un document présentant le fonctionnement général à travers un ou deux cas et faites des liens vers des pages contenant le détail complet des comportements.
